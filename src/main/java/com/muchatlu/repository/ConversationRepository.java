...
//    List<ConversationDto> getConversationForUser(@Param("id") Long id);
    ...
    @Query(value = "select t.id,t.user_id_from,t.username_from,t.avatar_from,t.user_id_to,t.username_to,t.avatar_to,msg.message,msg.username_from as msg_from,msg.timestamp from (select c.id,c.avatar_from,c.avatar_to,c.user_id_from,c.user_id_to,c.username_from,c.username_to,coalesce(max(m.id),0) msg_id from conversation c left join message m on c.id = m.conversation_id where c.user_id_to =:id group by c.id) t left join message msg on t.msg_id = msg.id",nativeQuery = true)
